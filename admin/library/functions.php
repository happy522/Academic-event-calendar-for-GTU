...
function doLogin()
{
	...
	$sql 	= "SELECT * FROM tbl_users WHERE (name = '$name' AND pwd = '$pwd') AND type='admin'";
	$result = dbQuery($sql);
...
function getBookingRecords(){
	 $cid=($_SESSION['calendar_fd_user']['id']); 
	  $username=($_SESSION['calendar_fd_user']['name']); 
	$per_page = 10;
	$page = (isset($_GET['page']) && $_GET['page'] != '') ? $_GET['page'] : 1;
	$start 	= ($page-1)*$per_page;
	$sql 	= "SELECT  u.name,u.pwd,u.id AS uid ,u.email,u.phone,r.ename,r.eres, r.rdate, r.status, r.comments,r.id  FROM tbl_reservations r ,tbl_users u WHERE u.name=r.sec_head_name ORDER BY r.id DESC LIMIT $start, $per_page";
	//echo $sql;
	$result = dbQuery($sql);
	$records = array();
